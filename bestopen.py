...
def submit():
    ...
    if request.method == 'POST':
        ...
        total_bet_count = 0
        total_bet_amount = 0
        with  open('beton_matrix.csv', 'w+',  encoding='UTF-8') as f:
            ...
            for bet in jdata["Bets"]:
                ...
                for h in header:
                    if h in betOn:
                        ...
                        f.write(str(odds[i] * unitAmount) + ",")
                        #row.append( unitAmount )
                        #f.write(str( unitAmount) + ",")
                        i+=1
                        total_bet_count += 1
...
if __name__ == "__main__":
    ...
    logging.info("column count : " +str(len(header)))
    ...
    numbers =row_numbers[row_numbers.columns[0]]
    print("numbers:",  numbers)
    row_numbers = row_numbers[row_numbers.columns[1:-1]]
    print("row_numbers:",  row_numbers)
    numbers_length = row_numbers.shape[0]
    numbers_answer = np.array(row_numbers[:-1]).flatten().astype(np.float32)
